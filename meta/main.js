...
function processCommits(data) {
  return d3
    ...
    .map(([commit, lines]) => {
      ...
      let ret = {
        id: commit,
        url: 'https://github.com/diegomenchaca/portfolio/commit/' + commit,
        author,
        date,
        time,
        timezone,
...
function renderCommitInfo(data, commits) {
  ...
  const [topAuthor] = d3.greatest(
    d3.rollups(data, (v) => v.length, (d) => d.author),
    ([, count]) => count,
  );
  dl.append('dt').text('Top author');
  dl.append('dd').text(topAuthor);
}
...
function onStepEnter(response) {
  ...
  commitProgress = timeScale(commitDatetime);
